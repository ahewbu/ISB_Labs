...
def frequency_bit_test(sequence) -> float:
    try:
        summary = 0
        for i in sequence:
            if int(i) == 1:
                ...
                summary -= 1
        summary = math.fabs(summary) / math.sqrt(len(sequence))
        p_value = math.erfc(summary / math.sqrt(2))
        return p_value
    except Exception as ex:
        logging.error(f"ZeroDivisionError: {ex.message}\n{ex.args}\n")


def ordinary_bit_test(sequence) -> float:
    try:
